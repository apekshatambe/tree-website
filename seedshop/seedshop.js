  ...
  const applyFilters = () => {
    ...
    if (resultsCount) {
      if (query && count === 0) {
        ...
        resultsCount.textContent = `Showing all ${count} seeds`;
      } else {
        resultsCount.textContent = `Showing ${count} of ${cards.length} seeds`;
      }
    }
    ...
    if (noResults) {
      ...
      noResults.textContent =
        query && count === 0
          ? `No seeds match "${query}". Try another search.`
          : "No seeds match your filters. Try adjusting your selection.";
    }
